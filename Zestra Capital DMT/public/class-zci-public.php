...
class ZCI_Plugin {
    ...
    private function add_sample_data() {
        ...
        foreach ( $indicators as $ind ) {
            ...
            if ( !$exists ) {
                ...
                // Add sample data points for the last 24 months
                for ( $i = 23; $i >= 0; $i-- ) {
                    $date = date( 'Y-m-d', strtotime( "-$i months" ) );
                    $base_value = 100 + rand( -10, 20 );
                    $value = $base_value + ( $i * 0.2 ) + ( rand( -5, 5 ) ); // Add some trend and randomness
    ...
    public function admin_enqueue_scripts( $hook ) {
        ...
        wp_enqueue_style( 'zci-admin', ZCI_PLUGIN_URL . 'admin/css/zci-admin.css', [], ZCI_VERSION );
        wp_enqueue_script( 'zci-admin', ZCI_PLUGIN_URL . 'admin/js/zci-admin.js', [ 'jquery' ], ZCI_VERSION, true );
    }
    ...
    public function shortcode_compare( $atts ) {
        ...
        // Add Chart.js and custom styles
